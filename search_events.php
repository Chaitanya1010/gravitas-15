...
while($t=mysqli_fetch_array($r))
{
	if($t[3]>$t[4]) // total_seats>filled_seats
	{
		if($t[5]!=0) // Not 0 means, whatever is the team strength given that has to be added
		{
			...
			echo "<TR><TD>$t[1]</TD><TD> $t[2]</TD><TD><LABEL ID= '$select_id'>$t[5]</LABEL></TD><TD><button class='green darken-3 btn-floating z-depth-1' id='$t[0]' onclick='add_to_cart(this.id)'><i class='material-icons'>add</i></button></TD></TR>";
		}
		else //variable team size
		{
			$select_id = $t[0]."select";
			$select="<SELECT style='width: 130px' id='$select_id' class='browser-default'><OPTION VALUE ='$t[6]' SELECTED>$t[6]</OPTION>";
			for($i = $t[6]+1; $i<=$t[7];$i++)
