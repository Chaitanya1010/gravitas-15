<?php
	...
	if((isset($_SESSION['name_fin']))&&(isset($_REQUEST['id'])))//session_variable verification
	{
		...
		$query_basic = "SELECT * FROM `basic_expenditure_info` where category = $cat_1 AND mode = 1";
		...
		if((mysqli_num_rows($result_basic)>0)&&($id==0))
		{
			...
			echo   'Name' . "\t". 'Purpose' . "\t". 'Amount' . "\t" . 'Phone number' ."\t" . 'Remarks '."\t". 'DD number' . "\t" . 'Branch Name' . "\t". 'Bank Name' . "\t". 'Issue Date' . "\n\n";
		...
		else if((mysqli_num_rows($result_basic)>0)&&($id==1))
		{
			...
			header( "Content-Type: application/vnd.ms-excel" );
			header( "Content-disposition: attachment; filename=$file_name" );
			echo   'Name' . "\t". 'Purpose' . "\t". 'Amount' . "\t" . 'Phone number' ."\t" . 'Email-Id' . "\t".'Remarks '."\t". 'DD number' . "\t" . 'Branch Name' . "\t". 'Bank Name' . "\t". 'Issue Date' . "\n\n";
