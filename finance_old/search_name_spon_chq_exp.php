<?php
	...
	if((isset($_SESSION['name_fin']))&&(isset($_REQUEST['cat'])))//session_variable verification
	{
		...
		$fifth_data="Remarks";
			...
			$sql_chq = "SELECT * FROM  `mode_cheque_expenditure` WHERE LOWER(cheque_number) LIKE '$chq_number%' and category=".$category.";";
				...
				while($arr_chq=mysqli_fetch_array($res_chq))
				{
					...
					//approving data
						...
						if($mode==55)
						{
							...
							{
								echo "<div id='button_spon_chq_exp_".$chq_id."'></br><button onclick='return approve_spon_chq_exp(".$chq_id.")'>Approve the Transaction</button></div></br>";
							}
