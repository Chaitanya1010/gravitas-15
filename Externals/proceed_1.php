<?php
session_start();
if((isset($_SESSION["id"]))&&($_REQUEST['numb']))
{
	require("sql_con.php");
	$cart=$_POST["cart"];
	$team=$_POST["team"];
	...
	if($cart!="")
	{
		...
		echo"<button class='btn waves-effect waves-light indigo darken-4' onclick='checkout()' id='proceed_2' name='proceed_2'><i class='material-icons left'>check_circle</i>Checkout </button></div></div></div>";
	}
}
