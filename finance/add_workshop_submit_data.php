<?php
	...
	if((isset($_SESSION['name_fin']))&&(isset($_REQUEST['mode'])))//session_variable verification
	{
		...
		$counter_flag=0;
			...
			$sql_basic_add="INSERT INTO `finance`.`basic_info` (`category`,`event_name`, `company_name`, `amount`, `phno`, `email_id`, `mode`, `remarks`) VALUES('4', '$event_name', '$company_name', '$amount', '$phno', '$email_id', '$selected', '$remarks');";
			$res_basic_add=mysqli_query($mysqli,$sql_basic_add);
			
			if($res_basic_add)
			{
				$get_inserted_id="SELECT `unique_id` FROM `basic_info` WHERE `event_name`='$event_name' and `company_name`='$company_name' and `amount`='$amount' and `mode`=$selected and `category`=4;";
		...
		if($selected==0)		//cash
		{
			...
			$note_1000=$_REQUEST['note_1000'];
					...
					$sql_add_cash="INSERT INTO `finance`.`mode_cash` (`note_1`, `note_2`, `note_5`, `note_10`, `note_20`, `note_50`, `note_100`, `note_500`, `note_1000`, `category`, `unique_id_basic`, `approval_1`, `approval_2`, `approval_3`) VALUES ('$note_1', '$note_2', '$note_5', '$note_10', '$note_20', '$note_50', '$note_100', '$note_500', '$note_1000',	'4', '$id', 0, 0, 0);";
		...
		{
			$sql_del_basic="DELETE * FROM `basic_info` WHERE unique_id='$i' and category='4';";
