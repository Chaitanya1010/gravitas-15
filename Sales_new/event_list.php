...
function reg_go(paid_status_1)
{
	...
	var e_1 = document.getElementById('hostel_type');
	hostel_name= e_1.options[e_1.selectedIndex].value;
	if(hostel_name==99)
	{
			Materialize.toast("Please Select the Hostel Name",3000,'rounded');
			return false;
	}
	...
	if(regno.match(pattern)&&(numb.length==10)&&(blck_name!='')&&(room_no!='')&&(email_id=="")&&(email_id.match(mailformat)))
	{
		for(i=1;i<=5;i++)
		{
			if(i==1)
...
<body>
  <main>
      ...
      <button onclick='return reg_go(1)' class="waves-effect waves-light indigo darken-2 btn z-depth-3">Pay Now<i class="material-icons right">send</i> </button>
      <button onclick='return reg_go(0)' class="waves-effect waves-light indigo darken-2 btn z-depth-3">Pay Later<i class="material-icons right">send</i> </button>
    </div>
		</div>
